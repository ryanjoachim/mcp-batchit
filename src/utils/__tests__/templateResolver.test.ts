import { resolveTemplates, clearTemplateCache, configureTemplateResolution, validateTemplate } from "../templateResolver.js"
import { resultsCache } from "../resultsCache.js"
import { registerPartial, unregisterPartial, clearPartials, registerPartials } from "../templatePartials.js"
...
describe("templateResolver", () => {
  ...
  describe("built-in string helpers", () => {
    it("uppercase", () => {
      ...
      expect(resolveTemplates(args)).toEqual({ content: "HELLO", template: undefined })
    })
    ...
    it("substring", () => {
      ...
      expect(resolveTemplates(args)).toEqual({ content: "ell", template: undefined })
    })

    it("replace", () => {
      const args = { template: "{{replace 'hello world' 'world' 'there'}}" }
      expect(resolveTemplates(args)).toEqual({ content: "hello there", template: undefined })
    })

    it("concat", () => {
      const args = { template: "{{concat 'hello' ' ' 'world'}}" }
      expect(resolveTemplates(args)).toEqual({ content: "hello world", template: undefined })
    })
  })
  ...
  describe("built-in conditional helpers", () => {
    it("eq returns true when equal", () => {
      ...
      expect(resolveTemplates(args)).toEqual({ content: "yes", template: undefined })
    })

    it("eq returns false when not equal", () => {
      const args = { template: "{{#if (eq 1 2)}}yes{{else}}no{{/if}}" }
      expect(resolveTemplates(args)).toEqual({ content: "no", template: undefined })
    })
  ...
  describe("built-in math helpers", () => {
    it("add", () => {
      ...
      expect(resolveTemplates(args)).toEqual({ content: "5", template: undefined })
    })

    it("divide by zero returns 0", () => {
      const args = { template: "{{divide 10 0}}" }
      expect(resolveTemplates(args)).toEqual({ content: "0", template: undefined })
    })

    it("round", () => {
      const args = { template: "{{round 3.7}}" }
      expect(resolveTemplates(args)).toEqual({ content: "4", template: undefined })
    })
  })
  ...
  describe("built-in collection helpers", () => {
    it("length", () => {
      ...
      expect(resolveTemplates({ template: "{{length items}}", content: context })).toEqual({
        content: "3",
        template: undefined,
      })
    })
    ...
    it("first", () => {
      const context = { items: [1, 2, 3] }
      expect(resolveTemplates({ template: "{{first items}}", content: context })).toEqual({
        content: "1",
        template: undefined,
      })
    })
    ...
    it("last", () => {
      const context = { items: [1, 2, 3] }
      expect(resolveTemplates({ template: "{{last items}}", content: context })).toEqual({
        content: "3",
        template: undefined,
      })
    })
    ...
    it("join", () => {
      ...
      expect(resolveTemplates({ template: "{{join items '-'}}", content: context })).toEqual({
        content: "a-b-c",
        template: undefined,
      })
    })
    ...
    it("includes true", () => {
      const args = { template: "{{#if (includes items 'b')}}found{{else}}not found{{/if}}" }
      const context = { items: ["a", "b", "c"] }
      expect(resolveTemplates({ template: args.template, content: context })).toEqual({
        content: "found",
        template: undefined,
      })
    })
    ...
    it("includes false", () => {
      const args = { template: "{{#if (includes items 'd')}}found{{else}}not found{{/if}}" }
      const context = { items: ["a", "b", "c"] }
      expect(resolveTemplates({ template: args.template, content: context })).toEqual({
        content: "not found",
        template: undefined,
  ...
  describe("built-in date/time helpers", () => {
    it("formatDate without format returns ISO string", () => {
      ...
      const result = resolveTemplates({ template: args.template, content: context })
      expect(result.template).toBeUndefined()
  ...
  describe("cache eviction", () => {
    it("evicts oldest entries when max size is exceeded", () => {
      const cache = new TemplateCache({ maxSize: 5, evictionRate: 0.2 })
      ...
      // Add new entry — should evict exactly 1: 'a' (oldest inserted)
  ...
  describe("template validation", () => {
    it("validates syntactically correct template with known helpers", () => {
      const result = validateTemplate("Hello {{uppercase name}}", { checkHelpers: true })
  ...
  describe("partials", () => {
    it("registers and unregisters partials", () => {
      registerPartial("greeting", "Hello {{uppercase name}}!")
      const args = { template: "{{> greeting}}", content: { name: "World" } }
      const result = resolveTemplates(args)
      expect(result.content).toBe("Hello WORLD!")
      unregisterPartial("greeting")
    })

    it("unregisterPartial removes partial", () => {
      registerPartial("greeting", "Hello {{name}}!")
      unregisterPartial("greeting")
      const result = validateTemplate("{{> greeting}}", { checkPartials: true })
      expect(result.valid).toBe(false)
      expect(result.errors[0].type).toBe("partial")
    })

    it("registerPartials registers multiple", () => {
      registerPartials({
        header: "== {{title}} ==",
        footer: "-- {{name}} --",
      })
      const args1 = { template: "{{> header}}", content: { title: "Welcome" } }
      const args2 = { template: "{{> footer}}", content: { name: "Ryan" } }
      expect(resolveTemplates(args1).content).toBe("== Welcome ==")
      expect(resolveTemplates(args2).content).toBe("-- Ryan --")
    })

    it("clearPartials removes all", () => {
      registerPartials({ one: "1", two: "2" })
      clearPartials()
      const result = validateTemplate("{{> one}}", { checkPartials: true })
      expect(result.valid).toBe(false)
    })
  })
  ...
  describe("validateBeforeExecution option", () => {
    ...
    it("succeeds on valid template when validation enabled", () => {
      configureTemplateResolution({ validateBeforeExecution: true })
      const args = { template: "Hello {{uppercase name}}", content: { name: "World" } }
      const result = resolveTemplates(args)
      expect(result.content).toBe("Hello WORLD")
      configureTemplateResolution({ validateBeforeExecution: false })
    })
